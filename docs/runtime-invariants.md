...
1. **ToolResult ordering** — tool results are appended to message history in
   ...
   message, regardless of completion order. (`run.rs` submission_order +
   completed_map reconstruction)
...
2. **ToolUse / ToolResult pairing** — every ToolUse block in the assistant
   ...
   always produced. (`executor.rs` catch_unwind, `run.rs` instant_failure)
...
7. **ToolStart before ToolDone/ToolError** — `ToolStart` is emitted before
   ...
   post-tool hooks run, in submission order. (`run.rs` emission_guard)
...
9. **Hook events in submission order** — `PostToolUse` and `PostToolFailure`
   hooks fire in submission order, not completion order. (`run.rs`
...
10. **Turn serialisation** — `Session::send()` acquires a semaphore permit.
    ...
    Error. (`session.rs` turn_guard + permit_slot)

11. **History atomicity on Done** — when `AgentEvent::Done` is yielded to
    ...
    (`session.rs` then closure, synchronous lock + async store.save)

12. **Error preserves history** — when a run ends with `AgentEvent::Error`,
    `session.messages` stays at its pre-send state. The dangling user
    message is not added to history. (`session.rs` — history is only
    updated inside the Done branch)

## Permission

13. **Deny rules are absolute** — a static deny rule cannot be overridden by
    session decisions, mode, or hooks. (`permission.rs` evaluation order:
    deny first)

14. **Session decisions cannot weaken static denials** — a user's
    `ApproveAlways` cannot override a builder-level `.deny_tool()`.
    (`permission.rs` — deny checked before session memory)

15. **HITL after LLM finishes** — permission prompts are never shown while
    the LLM is still streaming. Tool calls are collected during streaming
    and authorized after MessageEnd. (`run.rs` pending_auths + auth_tasks)

## Compression

16. **L1 before L2 before L3** — compression tiers are applied in order,
...
17. **Compression never loses the most recent messages** — L2 collapse and
...
18. **Emergency compression is bounded** — prompt-too-long triggers at most
    one emergency compression per iteration. If compression doesn't help,
    the error propagates. (`run.rs` — `continue` restarts the iteration,
    but the next attempt won't re-trigger emergency compression for the
    same request)

## Checkpoint

19. **Checkpoint is per-iteration** — a checkpoint is saved at the end of
    ...
    iteration, not mid-tool. (`run.rs` checkpoint save site)
