...
impl Tool for Lookup {
    ...
    fn description(&self) -> &str {
        "Look up a value by key (read-only)."
    }
...
async fn test_readonly_allowed(provider: Anthropic) {
    ...
    let mut stream = agent.stream("Look up the value for key 'x'.");
    let mut ran = false;
...
async fn test_mutating_blocked(provider: Anthropic) {
    ...
    let mut stream = agent.stream("Delete record id '42'.");
