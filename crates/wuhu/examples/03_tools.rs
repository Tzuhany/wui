...
impl Tool for SecretCode {
    fn name(&self) -> &str { "get_secret_code" }
    fn description(&self) -> &str { "Retrieve the current secret access code." }
    fn prompt(&self) -> String { "Returns the secret access code. Input: {}".into() }
...
impl Tool for AlwaysFails {
    ...
    fn description(&self) -> &str { "A tool that always returns an error." }
    fn prompt(&self) -> String { "Always fails. Input: {}".into() }
...
async fn test_success(provider: Anthropic) {
    ...
    let mut stream     = agent.stream("What is the current secret access code?").await;
    ...
    assert!(text.contains("WUHU-ALPHA-9274"), "response doesn't mention the code: {text}");
...
async fn test_failure(provider: Anthropic) {
    ...
    let agent = Agent::builder(provider)
        ...
        .model("claude-haiku-4-5-20251001")
        .tool(AlwaysFails)
        // Force only the first LLM call to use a tool; subsequent turns (the
        // error-reporting turn) are free to respond with text.
        .initial_extension("tool_choice", serde_json::json!({"type": "any"}))
        .permission(PermissionMode::Auto)
        .build();

    let mut stream    = agent.stream("Get the current system status.").await;
    let mut got_error = false;

    while let Some(event) = stream.next().await {
        match event {
            AgentEvent::ToolError { name, error, kind, ms, .. } => {
                assert_eq!(name, "always_fails");
                assert!(error.contains("something went wrong"), "unexpected error: {error}");
