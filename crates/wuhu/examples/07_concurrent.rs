...
impl Tool for SlowRead {
    fn name(&self) -> &str { "slow_read" }
    fn description(&self) -> &str { "A slow read operation that takes 500ms." }
    fn prompt(&self) -> String { "Input: {\"label\": \"...\"}. Sleeps 500ms, returns the label.".into() }
    ...
    fn is_concurrent_for(&self, _input: &Value) -> bool { true }
...
impl Tool for SlowWrite {
    fn name(&self) -> &str { "slow_write" }
    fn description(&self) -> &str { "A slow write operation that takes 400ms and must not run concurrently." }
    fn prompt(&self) -> String { "Input: {\"data\": \"...\"}. Sleeps 400ms, confirms write.".into() }
    ...
    fn is_concurrent_for(&self, _input: &Value) -> bool { false }
...
async fn test_concurrent(provider: Anthropic) {
    ...
    let mut stream = agent.stream(
        "Read both 'alpha' and 'beta' using slow_read. Call both tools."
    ).await;
    ...
    if starts.len() == 2 {
        ...
        if elapsed.as_millis() < 1200 {
            println!("    ✓ ran concurrently (~{}ms < 1200ms threshold)", elapsed.as_millis());
        } else {
            println!("    ⚠ elapsed {}ms — may have run serially (acceptable if LLM called separately)", elapsed.as_millis());
        }
    }
    assert_eq!(dones.len(), starts.len(), "mismatch: {} starts, {} dones", starts.len(), dones.len());
...
async fn test_serial(provider: Anthropic) {
    ...
    let mut stream = agent.stream(
        "Write 'one' and 'two' using slow_write. Call both tools."
    ).await;
    ...
    assert!(dones.len() >= 1, "no writes completed");
