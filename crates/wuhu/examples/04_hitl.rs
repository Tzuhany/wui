...
impl Tool for Stamp {
    ...
    fn description(&self) -> &str { "Return a timestamp string." }
    fn prompt(&self) -> String { "Returns 'stamped'. Input: {}".into() }
...
async fn test_approve(provider: Anthropic) {
    ...
    let mut stream      = agent.stream("Please stamp this document.").await;
...
async fn test_deny(provider: Anthropic) {
    ...
    let agent = Agent::builder(provider)
        .system("When asked to stamp something, call the stamp tool. If denied, say 'access denied'.")
        .model("claude-haiku-4-5-20251001")
        .tool(Stamp)
        .permission(PermissionMode::Ask)
        .build();
    ...
    while let Some(event) = stream.next().await {
        match event {
            ...
            AgentEvent::ToolDone { .. } => { tool_ran = true; }
...
async fn test_approve_always(provider: Anthropic) {
    ...
    assert!(count_after_first >= 1, "expected at least one control prompt");
