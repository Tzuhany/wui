...
use wuhu::prelude::*;
use wuhu::{FailureKind, HookEvent};
...
impl Tool for Echo {
    ...
    fn prompt(&self) -> String { "Input: {\"text\": \"...\"}".into() }
...
#[async_trait]
impl Tool for Loud {
    fn name(&self) -> &str { "loud" }
    fn description(&self) -> &str { "Return text in uppercase." }
    fn prompt(&self) -> String { "Input: {\"text\": \"...\"}".into() }
    fn input_schema(&self) -> Value {
        json!({ "type": "object", "properties": { "text": { "type": "string" } }, "required": ["text"] })
    }
...
async fn test_denylist(provider: Anthropic) {
    ...
    let agent = Agent::builder(provider)
        .system("When asked to be loud, call the loud tool. When asked to echo, call the echo tool.")
    ...
    let mut stream     = agent.stream("Please use the loud tool to say hello.").await;
...
async fn test_banned_words(provider: Anthropic) {
    ...
    let mut stream      = agent.stream("Echo the word: forbidden").await;
    let mut got_blocked = false;
